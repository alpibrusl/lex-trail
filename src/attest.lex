...
fn add(
  ...
  match sql.exec(log.db,
    "INSERT OR IGNORE INTO attestations(id, event_id, kind, payload_json, ts_ms) VALUES (?, ?, ?, ?, ?)",
    [PStr(att.id), PStr(att.event_id), PStr(att.kind), PStr(att.payload_json), PInt(att.ts_ms)]) {
  Err(e) => Err(e.message),
  Ok(_)  => Ok(att),
  }
}
...
fn chain(
  log      :: l.Log,
  event_id :: Str
) -> [sql] Result[List[Attestation], Str] {
...
fn decode_attest_row[R](row :: R) -> Attestation {
  { id:           opt_str(sql.get_str(row, "id")),
    ...
    payload_json: opt_str(sql.get_str(row, "payload_json")),
    ts_ms:        opt_int(sql.get_int(row, "ts_ms")) }
}
