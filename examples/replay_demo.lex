...
fn show_verify(l1 :: log.Log, l2 :: log.Log) -> [sql, io] Unit {
  ...
  let ok1 := match v1 { Ok(_) => true, Err(_) => false }
  let ok2 := match v2 { Ok(_) => true, Err(_) => false }
  let sym1 := if ok1 { "✓" } else { "✗" }
  let sym2 := if ok2 { "✓" } else { "✗" }
  let c1   := match v1 { Ok(n) => int.to_str(n), Err(_) => "?" }
  let c2   := match v2 { Ok(n) => int.to_str(n), Err(_) => "?" }
...
fn show_replay(l1 :: log.Log) -> [sql, io] Unit {
  ...
  # Get the head of log1 (most recently appended = leaf)
...
fn main() -> [io, sql, fs_write, time] Unit {
  ...
  # Open two independent in-memory logs (one per session)
