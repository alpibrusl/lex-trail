...
fn task_payload(task_id :: Str, extra :: Str) -> Str {
  str.concat("{\"task_id\":\"", str.concat(task_id, str.concat("\",", str.concat(extra, "}"))))
}
...
fn test_task_replay_finds_matching() -> [sql, fs_write, time] Result[Unit, Str] {
  ...
  match log.open_memory() {
    ...
    Ok(l) => match log.append(l, k.a2a_task_received(), None, task_payload(task_id, "\"skill\":\"summarize\"")) {
      Err(e) => Err(str.concat("append1 failed: ", e)),
      Ok(_) => match log.append(l, k.llm_step(), None, task_payload(task_id, "\"tokens_in\":10,\"tokens_out\":5,\"tool_calls\":[]")) {
        Err(e) => Err(str.concat("append2 failed: ", e)),
        Ok(_) => match replay.task(l, task_id) {
          Err(e) => Err(str.concat("replay failed: ", e)),
          Ok(evts) => if list.len(evts) == 2 {
            Ok(())
          } else {
            Err(str.concat("expected 2 events, got ", int.to_str(list.len(evts))))
          },
        },
      },
    },
...
fn test_task_replay_excludes_other_tasks() -> [sql, fs_write, time] Result[Unit, Str] {
  ...
  match log.open_memory() {
    Err(e) => Err(str.concat("open failed: ", e)),
    Ok(l) => match log.append(l, k.a2a_task_received(), None, task_payload(task_id, "\"skill\":\"translate\"")) {
      Err(e) => Err(str.concat("append wanted failed: ", e)),
      Ok(_) => match log.append(l, k.cap_invoked(), None, task_payload(other_id, "\"capability\":\"search\"")) {
...
fn test_walk_chain_single_event() -> [sql, fs_write, time] Result[Unit, Str] {
  match log.open_memory() {
    Err(e) => Err(str.concat("open failed: ", e)),
    Ok(l) => match log.append(l, "k", None, "{}") {
      ...
      Ok(evt) => {
        ...
        if list.len(chain) == 1 {
          ...
          Err("expected chain of length 1")
        }
      },
    },
  }
...
fn test_walk_chain_parent_link() -> [sql, fs_write, time] Result[Unit, Str] {
  match log.open_memory() {
    Err(e) => Err(str.concat("open failed: ", e)),
    Ok(l) => match log.append(l, "root", None, "{}") {
      Err(e) => Err(str.concat("root append failed: ", e)),
      Ok(root) => match log.append(l, "child", Some(root.id), "{}") {
        ...
        Ok(child) => {
          ...
          if list.len(chain) == 2 {
            ...
            Err(str.concat("expected chain of 2, got ", int.to_str(list.len(chain))))
          }
        },
      },
    },
...
fn run_all() -> [sql, fs_write, time] Unit {
  let results := [test_task_replay_finds_matching(), test_task_replay_excludes_other_tasks(), test_walk_chain_single_event(), test_walk_chain_parent_link()]
