...
fn test_compute_id_changes_on_kind() -> Result[Unit, Str] {
  ...
  if id1 != id2 {
    ...
    Err("different kinds produced same id")
  }
}
...
fn test_compute_id_changes_on_parent() -> Result[Unit, Str] {
  ...
  if id1 != id2 {
    ...
    Err("different parents produced same id")
  }
}
...
fn test_compute_id_changes_on_payload() -> Result[Unit, Str] {
  ...
  if id1 != id2 {
    ...
    Err("different payloads produced same id")
  }
}
...
fn test_make_payload() -> Result[Unit, Str] {
  ...
  if evt.payload_json == "{\"x\":1}" {
    Ok(())
  } else {
    Err(str.concat("wrong payload: ", evt.payload_json))
  }
}
...
fn test_is_valid_fresh() -> Result[Unit, Str] {
  ...
  if ev.is_valid(evt) {
    ...
    Err("freshly made event failed is_valid")
  }
}

fn test_is_valid_rejects_tampered_kind() -> Result[Unit, Str] {
  let evt := ev.make("spec.allowed", None, "{}", 1)
  let bad := { id: evt.id, kind: "spec.denied", parent: evt.parent, payload_json: evt.payload_json, ts_ms: evt.ts_ms }
  if not ev.is_valid(bad) {
    Ok(())
  } else {
    Err("tampered event passed is_valid")
  }
}

fn run_all() -> Unit {
  let results := [test_compute_id_deterministic(), test_compute_id_changes_on_kind(), test_compute_id_changes_on_parent(), test_compute_id_changes_on_payload(), test_compute_id_changes_on_ts_ms(), test_make_kind(), test_make_payload(), test_make_ts_ms(), test_make_parent_none(), test_make_parent_some(), test_is_valid_fresh(), test_is_valid_rejects_tampered_kind()]
